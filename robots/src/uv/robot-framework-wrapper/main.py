...
- Passing variables to Robot Framework
- Executing robot files via subprocess
- Extracting and reporting results (output.xml, logs)
...
def run_robot_file(
    ...
    """Execute a Robot Framework file via subprocess.
    
    This is the core function that runs Robot Framework with proper:
...
def main():
    ...
    try:
        ...
        if item_count == 0:
            ...
            logger.info(f"Robot Framework wrapper completed successfully ({item_count} items)")
        
    except Exception as e:
        logger.error(f"Fatal error in wrapper: {e}", exc_info=True)
        raise
