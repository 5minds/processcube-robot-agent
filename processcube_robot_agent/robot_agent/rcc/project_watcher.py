...
class RobotsFileSystemEventHandler(FileSystemEventHandler):
    ...
    def on_any_event(self, event) -> None:
        ...
        Handles:
            - Finding robot.yaml for changed files
        ...
        def find_robot_yaml(path: Path) -> Optional[Path]:
            """Recursively find parent robot.yaml file.

            Args:
                path: File or directory path to start search.

            Returns:
                Path to robot.yaml if found, None otherwise.
            """
            logger.debug(f"find_robot_yaml {path}")
        ...
        try:
            ...
            if robot_yaml is not None:
                try:
                    ...
                    logger.error(f"Failed to pack or install robot {robot_yaml}: {e}", exc_info=True)
            else:
                logger.debug(f"No robot.yaml found for changed path {changed_path}")
        except Exception as e:
